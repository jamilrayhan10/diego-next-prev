const lineMarker = () => {
	if (typeof window !== "undefined") {
		function tp_tab_line_2() {
			...
			// Simulate a click event on the first item when the page loads
			indicator({ target: item[0] });
			...
			if (activeNav) {
				...
				var activePadLeft = parseFloat(
					window.getComputedStyle(activeNav).paddingLeft
				);
				var activePadRight = parseFloat(
					window.getComputedStyle(activeNav).paddingRight
				);
				...
				function anchorWidthCounter() {
					...
					item.forEach(function (elem) {
						...
						var aPadLeft = parseFloat(
							window.getComputedStyle(elem).paddingLeft
						);
						var aPadRight = parseFloat(
							window.getComputedStyle(elem).paddingRight
						);
